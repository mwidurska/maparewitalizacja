...
info.update = function (props) {
    ...
    if (props) {
        if (props.PRtype) {
            ...
            '</p><div class="photo"><a href="'+props.picture+'"target="_blank" title="Zdjęcie otworzy się w nowej karcie"> <img src="'+props.picture+'" alt="'+props.pictureAlt+'"> </a></div>';
        } else {
            this._div.innerHTML = '<div class="'+ props.sphere + '"><h2>Informacje o projekcie</h2>' + '<p><b>Tytuł projektu: ' + props.title + '</b></p></div><p>' + props.description +
            '</p></div>';
        }
...
displayCommune(bialystokCity);
displayCommune(bielskPodlaskiCity);
displayCommune(bockiCommune);
displayCommune(branskCity);
displayCommune(choroszczCommune);
displayCommune(czarnaBialostockaCommune);
displayCommune(czyzeCommune);
displayCommune(czyzewCommune);
displayCommune(dobrzyniewoDuzeCommune);
